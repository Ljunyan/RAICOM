...
echo -e ""
echo -e "${Info} 控制 spark 移动到 ${Yellow_background_prefix}分拣区对角${Font_color_suffix}\c"
echo && stty erase '^H' && read -p "       然后在这个界面内按回车..." 
rostopic pub /mark_nav std_msgs/String "data: 'learn Sorting_duijiao'" -1 >/dev/null
echo -e "${Info} 地点名为 ${Yellow_background_prefix}Sorting_duijiao${Font_color_suffix}"
