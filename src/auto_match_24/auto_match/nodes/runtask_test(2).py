...
class SwiftProInterface:
    ...
    def set_pose_drop(self, x, y, z):
        ...
        self.arm_position_pub.publish(pos)
        rospy.sleep(1.5)
    def set_pose_no_delay(self, x, y, z):
        '''
        发布机械臂运动位置
        '''
        pos = position()
...
class CamAction:
    ...
    def detector(self):
        ...
        # 提取
        for obj in obj_array.detections.detections:
            if obj.results[0].id not in obj_dist:
                obj_dist[obj.results[0].id] = []
            obj_dist[obj.results[0].id].append([obj.bbox.center.x, obj.bbox.center.y, 0])
        ...
        #             if value[1] > 200:
        #                 cube_list.append([key, value])
        for key, values in obj_dist.items():
            if key in self.search_id:
                for value in values:
                    cube_list.append([key, value])
...
class ArmAction:
    ...
    def grasp(self):
        ...
        if i==2:
            cube_list = cube_list[:2]
            cube_list = sorted(cube_list, key=lambda x: x[1][0],reverse=False) # x坐标排序
        if i==4:
            cube_list = [item for item in cube_list if item[1][0] < 500]
            cube_list = [item for item in cube_list if item[1][0] > 250]
        if i==5:
            cube_list = [item for item in cube_list if item[1][0] < 500]
        if i==6:
            cube_list = [item for item in cube_list if item[1][0] > 200]
        if i==7:
            cube_list = sorted(cube_list, key=lambda x: x[1][0],reverse=False) # x坐标排序
        ...
        # 机械臂移动到目标位置上方
        self.interface.set_pose_no_delay(x, y-20, z + 80)
        rospy.sleep(0.5)
        ...
        # 机械臂移动到目标位置
        self.interface.set_pose(x+9, y, z)
        # rospy.sleep(0.5)
        ...
        print(f"我把物品抬起来了")
        print(cube_list[0][0])
        self.interface.set_pose_no_delay(x, y, z + 120)
        # rospy.sleep(1)
        # r1.sleep()
    ...
    def drop(self, check=True,global_num=1):
        # global b_third_pos_x
        # global b_third_pos_y
        # global c_third_pos_x
        # global c_third_pos_y
        # global d_third_pos_x
        # global d_third_pos_y
        '''
        ...
        if (check):
            ...
            if len(cube_list) > 0 and global_num==2:
                cube_list = [item for item in cube_list if item[1][1] < 320]
                x = self.x_kb[0] * cube_list[0][1][1] + self.x_kb[1]
                y = self.y_kb[0] * cube_list[0][1][0] + self.y_kb[1]
                z = 77 # 120
                # print("sec ceng")
                # if global_BCD=="Collection_B":
                #     b_third_pos_x=cube_list[0][1][1]
                #     b_third_pos_y=cube_list[0][1][0]
                # elif global_BCD=="Collection_C":
                #     c_third_pos_x=cube_list[0][1][1]
                #     c_third_pos_y=cube_list[0][1][0]
                # elif global_BCD=="Collection_D":
                #     d_third_pos_x=cube_list[0][1][1]
                #     d_third_pos_y=cube_list[0][1][0]
            elif global_num==3:
                ...
                #     x=d_third_pos_x
                #     y=d_third_pos_y
                cube_list = [item for item in cube_list if item[1][1] < 340]
                #self.interface.set_pose_drop(210, 0, 172)
                self.interface.set_pose_drop(210, 0, 172)
                

                if cube_list[0][1][1] > 150:
                    cube_list[0][1][1]=cube_list[0][1][1]+48
                else:
                    cube_list[0][1][1]=cube_list[0][1][1]+60


                x = self.x_kb[0] * cube_list[0][1][1] + self.x_kb[1] 
                y = self.y_kb[0] * (cube_list[0][1][0] + 5) + self.y_kb[1]
                z = 170

        # 默认放置位置
        self.interface.set_pose_drop(x, y, z)
        rospy.sleep(1)
        ...
        if global_num==3:
            rospy.sleep(1)
            self.interface.set_pose_drop(190, 95, 173)
            self.interface.set_pose_drop(190, 140, 173)
    ...
    def arm_home(self, block=False):
        ...
        收起机械臂(无物品)
        '''
        self.interface.set_pose_no_delay(110, 0, 35)
        if block:
            rospy.sleep(2)
    ...
    def arm_grasp_ready(self, block=False):
        ...
        '''
        # self.interface.set_pose(90, 215, 150)
        self.interface.set_pose_no_delay(90, 190, 120)
        if block:
            rospy.sleep(2)
    ...
    def arm_grasp_first(self, block=False):
        self.interface.set_pose_no_delay(150, 0, 120)
        #rospy.sleep(1)
        if block:
            rospy.sleep(2)
    def arm_grasp_third(self, block=False):
        self.interface.set_pose_no_delay(150, 0, 120)
        self.interface.set_pose_no_delay(70, 200, 170)
        #rospy.sleep(1)
        if block:
            rospy.sleep(2)
    def arm_grasp_test(self, block=False):
        self.interface.set_pose_drop(210, 0, 172)
        rospy.sleep(2)
        self.interface.set_pose_drop(190, 140, 173)
...
class AutoAction:
    ...
    def set_carpose(self):#���ݶ�λ��ά���������λ��
        ...
        while tag.o_w>0.58+0.0 or tag.o_w<0.58-0.02:
            if tag.o_w>0.58+0.02 and tag.o_w>0:
        ...
        while tag.z>0.60+0.025 or tag.z<0.60-0.010:
            if tag.z>0.60+0.030:
            ...
            if tag.z<0.60-0.010:
                self.robot.step_back(0.007,-0.1)
                time.sleep(1)
            if tag.z>0.60+0.025:
                self.robot.step_go(0.005,0.1)
                time.sleep(1)
...
########################################################################################
    ...
    def run_task(self):
        ...
        while True:
            ...
            rospy.sleep(1)
            if i==2:
                self.robot.rotate_robot(12)
                self.robot.step_go(0.0845,0.1)
            if i==3:
                self.robot.rotate_robot(-10)
                self.robot.step_go(0.092,0.1)
                
            if i==4:
                self.robot.rotate_robot(4)
                self.robot.step_go(0.15,0.1)
            if i==5:
                self.robot.rotate_robot(17)
                self.robot.step_go(0.235,0.2)
            
            if i==6:
                self.robot.rotate_robot(-15)
                self.robot.step_go(0.21,0.2)
                
            if i==7:
                self.robot.rotate_robot(12)
                self.robot.step_go(0.285,0.2)
            if i==8:
                self.robot.rotate_robot(-6)
                self.robot.step_go(0.265,0.2)
            ...
            if ret: # 判断是否成功到达目标点
                ...
                print("========向后退一点===== ")
                if i==4:
                    self.robot.step_back(0.18,-0.5)  # 后退
                elif i==5 or i==6 or i==7 or i==8:
                    self.robot.step_back(0.24,-0.5)  # 后退
                elif i==2:
                    self.robot.step_back(0.13,-0.5)  # 后退
                elif i==0 or i==1 or i==3:
                    self.robot.step_back(0.08,-0.5)  # 后退
                if(item_type==class_list[0][0] and sorting_name == "Sorting_W"):
                    self.robot.rotate_robot(52)
                ...
                if(item_type==class_list[1][0] and sorting_name == "Sorting_W"):
                    self.robot.rotate_robot(-59)
                    self.robot.step_go(0.5,0.5) 
                    self.robot.rotate_robot(35)
                    self.robot.step_go(0.75,0.5)
                if (item_type == class_list[2][0] and sorting_name == "Sorting_W"):
                    self.robot.rotate_robot(-105)
                    self.robot.step_go(0.43, 0.5)

                if i != 0:
                    if item_type == class_list[0][0]: # duijiao TO B
                        self.robot.rotate_robot(-62)
                        self.robot.step_go(0.83, 0.5)
                    if item_type == class_list[1][0]: # duijiao TO C
                        self.robot.rotate_robot(-180)
                        self.robot.step_go(0.24, 0.5)
                    if item_type == class_list[2][0]: # duijiao TO D
                        self.robot.rotate_robot(63)
                        self.robot.step_go(0.82, 0.5)
            ...
            elif ((global_B == 2) and (item_type==class_list[0][0]) or (global_C == 2) and (item_type==class_list[1][0]) or (global_D == 2) and (item_type==class_list[2][0])):
                self.arm.arm_grasp_third()
                flat=1
            else:
            ...
            if ret:
                if flat:
                    while(1):
                        cube_list = self.cam.detector()
                        cube_list = [item for item in cube_list if item[1][1] < 340]
                        if cube_list[0][1][1] < 145:
                            # print("forwards")
                            # print(cube_list[0][1][1])
                            self.robot.step_go(0.015,0.05)
                            rospy.sleep(0.5)
                        elif cube_list[0][1][1] < 185:
                            # print("forwards")
                            # print(cube_list[0][1][1])
                            self.robot.step_go(0.01,0.05)
                            rospy.sleep(1)
                        elif cube_list[0][1][1] > 225:
                            # print("forwards")
                            # print(cube_list[0][1][1])
                            self.robot.step_back(0.01,-0.05)
                            rospy.sleep(1)
                        else:
                            # print("final y------------------------------")
                            # print(cube_list[0][1][1])
                ...
                #self.arm.drop()  # 放下物品
                self.robot.step_back(0.1,-0.3)  # 后退
                self.arm.arm_grasp_ready()  # 移动机械臂到其他地方
                # rospy.sleep(0.5)

                if global_BCD=="Collection_B":
                    self.robot.rotate_robot(-152)
                    self.robot.step_go(0.70,0.5)
                elif global_BCD=="Collection_C":
                    self.robot.rotate_robot(-195)
                    self.robot.step_go(0.2,0.4)
                elif global_BCD=="Collection_D":
                    self.robot.rotate_robot(136)
                    self.robot.step_go(0.77,0.5)
