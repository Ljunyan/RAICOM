...
class CamAction:
    ...
    def detector_class(self):
        ...
        #     if((class_list[1][1][1] - class_list[0][1][1]) < 180): # ���忿�� or ��
        #         if(class_list[0][1][1] < 250): # ûʶ�𵽵������ڵ�����
        #             third_class = [miss_class, 0, 0]
        #             class_list.append(third_class)
        #         else: # ûʶ�𵽵������ڵ�һ��
        #             first_class = [miss_class, 0, 0]
        #             class_list = first_class + class_id
        #     else:   # ûʶ�𵽵��������м�
        #         middle_class = [miss_class, 0, 0]
        #         class_list = class_list[0] + middle_class + class_list[1]
...
class ArmAction:
    ...
    def grasp(self):
        ...
        #cube_list = sorted(cube_list, key=lambda x: x[1][1]) # y坐标排序
        ...
        # 机械臂移动到目标位置上方
        self.interface.set_pose(x, y, z + 70)
        rospy.sleep(1)
    ...
    def arm_grasp_ready(self, block=False):
        ...
        '''
        self.interface.set_pose(60, 160, 160)  
        if block:
            rospy.sleep(3)
...
class AutoAction:
    ...
    def set_carpose(self):#���ݶ�λ��ά���������λ��
        ...
        while tag.o_w>0.58+0.01 or tag.o_w<0.58-0.01:
            if tag.o_w>0.58+0.01 and tag.o_w>0:
                self.robot.rotate_robot(4)
                time.sleep(0.5)
            if tag.o_w<0.58-0.01 or tag.o_w<0:
                self.robot.rotate_robot(-4)
                time.sleep(0.5)
        ...
        while tag.z>0.58+0.01 or tag.z<0.58-0.01:
            if tag.z>0.58+0.01:
                self.robot.step_go(0.01,0.1)
                time.sleep(0.5)
            if tag.z<0.58-0.01:
                self.robot.step_back(0.01,-0.2)
                time.sleep(0.5)
            print("tag.z: %f" % tag.z)
            time.sleep(4)
        print("tag.z: %f" % tag.z)
    ...
    def run_task(self):
        ...
        while True:
            ...
            if ret: # 判断是否成功到达目标点
                ...
                print("========向后退一点===== ")
                self.robot.step_back(0.12,-0.5)  # 后退
            ...
            if ret: 
                ...
                if global_BCD=="Collection_B" and sorting_name == "Sorting_W":
                    self.robot.rotate_robot(135)
                    self.robot.step_go(1.5,0.5)
                if global_BCD=="Collection_C" and sorting_name == "Sorting_W":
                    self.robot.rotate_robot(-135)
                    self.robot.step_go(1.5,0.5)
                if global_BCD=="Collection_B" and sorting_name == "Sorting_S":
                    self.robot.rotate_robot(-135)
                    self.robot.step_go(1.5,0.5)
                if global_BCD=="Collection_D" and sorting_name == "Sorting_E":
                    self.robot.rotate_robot(135)
                    self.robot.step_go(1.5,0.5)
                if global_BCD=="Collection_D" and sorting_name == "Sorting_N":
                    self.robot.rotate_robot(-135)
                    self.robot.step_go(1.5,0.5)
                if global_BCD=="Collection_C" and sorting_name == "Sorting_N":
                    self.robot.rotate_robot(135)
                    self.robot.step_go(1.5,0.5)
                if self.stop_flag: return
            else:
