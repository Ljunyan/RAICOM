...
PATH_TO_CKPT = MODEL_PATH + '/saved_model/saved_model.pb'
...
global inv_scale
class Detector:
    def resize_image(input,output,scale):
    ...
    def image_cb(self, data):
        ...
        scale=1.4
        M=np.float32([[scale,0,(1-scale)*center[0]],[0,scale,(1-scale)*center[1]]])
        image=cv2.warpAffine(image,M,(width,height))
        global inv_M
        global inv_scale
        inv_scale = 1/ scale
        inv_M = np.float32([[inv_scale,0,(1-inv_scale) * center[0]],
			    [0,inv_scale,(1-inv_scale)*center[1]]])
        # the array based representation of the image will be used later in order to prepare the
        # result image with boxes and labels on it.
        image_np = np.asarray(image) # 放开注释
        # Expand dimensions since the model expects images to have shape: [1, None, None, 3]
        image_np_expanded = np.expand_dims(image_np, axis=0) # 放开注释
    ...
    def object_predict(self,object_data, header, image_np,image):
        ...
        obj.bbox.center.x = int(np.dot(inv_M, [obj.bbox.center.x,obj.bbox.center.y, 1])[0])
        obj.bbox.center.y = int(np.dot(inv_M, [obj.bbox.center.x,obj.bbox.center.y, 1])[1])
        return obj
