...
class GraspObject():
    ...
    def __init__(self):
        ...
        package_path = os.path.join(rospack.get_path('semi_auto_match_24'))          # 获取功能包路径
    ...
    def grasp_cp(self, msg):
        ...
        if msg.data == '0':            
            ...
            self.grasp(self.water_x,self.water_y)
    ...
    def detector(self):
        ...
        try:
            obj_array = rospy.wait_for_message("/objects", Detection2DArray, timeout=None)
            print(len(obj_array.detections))
            if len(obj_array.detections) == 0:
                obj_array = rospy.wait_for_message("/objects_reverse", Detection2DArray, timeout=None)
        except Exception:
            cube_list.clear()
            return cube_list
